...
describe('processExcelFile', () => {
    ...
    it('should throw an error if date format conversion fails', () => {
        ...
        expect(errorService.createError).toHaveBeenCalledWith(400, 'Date format conversion error');
    });

    it('should throw an error if input data validation fails', () => {
        const fileDataPath = path.join(__dirname, '../../file.xlsx');
        const binaryData = fs.readFileSync(fileDataPath);
        const expectedInputParsedPath = path.join(__dirname, './mock/default_with_headings.json');
        const expectedInputParsed = JSON.parse(fs.readFileSync(expectedInputParsedPath));

        const invoicingMonthQuery = '2023-09';
        const validationError = [{ message: 'Validation error 1' }, { message: 'Validation error 2' }];

        extractDataService.extractDataFromExcel.mockReturnValueOnce(expectedInputParsed);
        ...
        expect(errorService.createError).toHaveBeenCalledWith(400, SERVICE_ERROR_MESSAGES.WRONG_INPUT, ['Validation error 1', 'Validation error 2']);
    });
